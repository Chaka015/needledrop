...
export default function AudioSetupEditor({ initial }: { initial: AudioSetup | null }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const res = await fetch("/api/setup", {
      ...
      body: JSON.stringify({ turntable: turntable || null, preamp: preamp || null, speakers: speakers || null }),
    });
    ...
    <div>
      {!editing ? (
        <div className="p-5 space-y-4 text-sm" style={{ backgroundColor: C.surface, border: `1px solid ${C.border}` }}>
          {hasSetup ? (
            <>
              {turntable && <SetupItem icon="💿" label="TURNTABLE" value={turntable} />}
              {preamp && <SetupItem icon="🎚️" label="PRE-AMP" value={preamp} />}
              {speakers && <SetupItem icon="🔊" label="SPEAKERS" value={speakers} />}
            </>
          ) : (
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4" style={{ backgroundColor: C.surface, border: `1px solid ${C.border}` }}>
          {[
            { icon: "💿", label: "TURNTABLE", value: turntable, set: setTurntable },
            { icon: "🎚️", label: "PRE-AMP", value: preamp, set: setPreamp },
            { icon: "🔊", label: "SPEAKERS", value: speakers, set: setSpeakers },
          ].map((field) => (
            <div key={field.label}>
              ...
              <input
                ...
                placeholder={`e.g. ${field.label === "TURNTABLE" ? "Technics SL-1200" : field.label === "PRE-AMP" ? "Schiit Mani" : "KRK Rokit 5"}`}
...
function SetupItem({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div>
      <div className="text-xs font-mono mb-0.5" style={{ color: C.subtle }}>{icon} {label}</div>
