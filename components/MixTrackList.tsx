      ...
      {items.map((item, idx) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <Link href={`/album/${encodeURIComponent(item.album.discogsId)}`} className="text-sm font-semibold truncate block hover:underline" style={{ color: C.text }}>{item.album.title}</Link>
            {item.album.artistMbid ? (
              <Link href={`/artist/${item.album.artistMbid}`} className="text-xs font-mono truncate block hover:underline" style={{ color: C.muted }}>{item.album.artist}</Link>
            ) : (
